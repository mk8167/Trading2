...
function renderSignalTab(d) {
  ...
  $('sigCard').innerHTML = `
    ...
    ${d.symbol?.otc ? '<div class="sub" style="margin-top:6px;color:#f5c33b">⚠ OTC pair — prices are broker-generated, not exchange-traded</div>' : ''}
    ${d.symbol?.source && d.symbol.source !== 'quotex' ? `<div class="sub" style="margin-top:6px;color:#f5c33b">⚠ Feed: ${esc(d.symbol.source)} (delayed proxy — the broker feed is not connected)</div>` : ''}
...
/**
 * Bankroll + the limits of what this ledger can claim.
 *
 * Two things belong together here. The bankroll is the guard: it stops trading
 * once realized losses have eaten the balance the user typed in, and it has to
 * be visible or the user will just see signals quietly stop. The honesty note
 * is the other half — every number on this tab is computed from OUR feed, and
 * a broker settles from its own, so a win rate read here is an estimate of a
 * strategy, never a promise about the broker's payouts.
 */
function renderBankroll(d) {
  const el = $('bankroll');
  if (!el) return;
  const b = d.bankroll;
  if (!b || !Number.isFinite(b.current)) { el.innerHTML = ''; return; }
  const money = (v) => `${v >= 0 ? '' : '-'}${Math.abs(v).toFixed(2)}`;
  const cls = b.realized > 0 ? 'pos' : b.realized < 0 ? 'neg' : 'neu';

  const sym = d.symbol;
  const proxy = sym && sym.source && sym.source !== 'quotex';
  const notes = [];
  notes.push(
    'This is a <b>paper ledger</b>. Every trade here is settled from this extension&rsquo;s own price feed, ' +
    'at the bar closest to expiry &mdash; not from your broker&rsquo;s settlement feed. The broker may settle the ' +
    'same trade at a different price, so treat these results as a measurement of the strategy, never as a ' +
    'prediction of what your account will do.'
  );
  if (sym?.otc) {
    notes.push(
      `<b>${esc(sym.pretty || sym.sym)} is an OTC pair.</b> Its prices are generated by the broker rather than ` +
      'traded on an exchange, so there is no independent feed to check them against. Indicative only.'
    );
  }
  if (proxy) {
    notes.push(
      `Prices for <b>${esc(sym.pretty || sym.sym)}</b> currently come from <b>${esc(sym.source)}</b>, a delayed ` +
      'proxy standing in because the broker&rsquo;s own feed is not connected. Settlements against a proxy are ' +
      'approximations, and for crypto the proxy quotes a different underlying (USDT, not USD).'
    );
  }

  el.innerHTML = `
    ${b.canTrade ? '' : `<div class="banner stop">⛔ <b>Trading stopped.</b> ${esc(b.reason)}. Raise the starting balance in Settings, or reset the journal if this run was only a test.</div>`}
    <div class="statgrid">
      <div class="stat"><div class="k">Bankroll</div><div class="v ${cls}">${money(b.current)}</div><div class="s">${money(b.starting)} start ${b.realized >= 0 ? '+' : ''}${b.realized.toFixed(2)} P/L</div></div>
      <div class="stat"><div class="k">Next stake</div><div class="v ${b.canTrade ? '' : 'neg'}">${b.stake.toFixed(2)}</div><div class="s">${b.riskPct}% of bankroll</div></div>
      <div class="stat"><div class="k">Guard</div><div class="v ${b.canTrade ? 'pos' : 'neg'}">${b.canTrade ? 'armed' : 'halted'}</div><div class="s">stops at zero bankroll</div></div>
    </div>
    <div class="banner ${sym?.otc || proxy ? 'warn' : ''}" style="margin-top:8px;margin-bottom:0">${notes.join('<br><br>')}</div>
  `;
}
...
function renderSettings(d) {
  ...
  if (!settingsLoaded || state.tab === 'settings') {
    // Every settings group is optional here. A settings object written by an
    // older version — or half-written when the worker was killed mid-save —
    // must not throw: renderSettings runs last in apply(), and an exception
    // would abort the frame and leave every card showing the PREVIOUS
    // payload's numbers while looking perfectly live.
    const st = s.strategy || {};
    const al = s.alerts || {};
    const fd = s.feeds || {};
    set('sBal', s.balance); set('sRisk', s.riskPct); set('sPay', s.payout);
    set('sMinScore', st.minScore); set('sMinPay', st.minPayout);
    // Math.round(undefined / 1000) is NaN, and `v ?? ''` does not catch NaN.
    set('sCd', Number.isFinite(st.cooldownMs) ? Math.round(st.cooldownMs / 1000) : '');
    set('sMaxVol', st.maxVolatility);
    set('sGateN', st.gateLookback); set('sStreak', st.maxLossStreak);
