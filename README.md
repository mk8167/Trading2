...
| + | Coverage: `patterns`, `feeds`, `settlement`, `bankroll`, `bootstrap`, `panel` and `ui-ids` had **no tests at all**. | **423 tests across 20 files** (was 234 across 11). Dead code removed: `candles.TFS`, `candles.makeCandle`, `journal.openExposure`, `journal.byHour`. |
...
- `chart` — the canvas renderer against a recording 2D context: HiDPI scaling, bodies stay
  inside the plot, overlays widen the scale instead of clipping, out-of-range levels are
  skipped, markers land on visible candles only, crosshair inverse-maps pixel → price, and
